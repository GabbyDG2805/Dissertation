...
<html lang="en">
	...
	<head>
		...
		<title>Builds of Legends | Guide</title>
	</head>
			...
			//We then check if we had atleast 1 result from the database
				...
				//We can spit out the data by refering to our database column names
					...
					echo'<div class="page">';
						...
						<div class="columnc"><h2>'.$row['Champion'].'</h2></div>
						<div class="columnc"><h2>'.$row['Patch'].'</h2></div>';
						...
						$resultCheckAuthor = mysqli_num_rows($resultAuthor);
							if ($resultCheckAuthor > 0) {
								while ($rowAuthor = mysqli_fetch_assoc($resultAuthor)) {
									...
									$level = $rowAuthor['Lvl'];
									echo'<div class="columnc"><h2>by '.$rowAuthor['Username'].'</h2></div>';
									if($region == 'EUW') {
						...
						echo'<h4 class="subheading">Core Build</h4>';
						...
						echo'<h4 class="subheading">Situational Builds</h4>';
						$situations = json_decode($row['Situations']);
